...
public class Page3SportPage extends JFrame implements ActionListener{
    ...
    public Page3SportPage(){
        ...
        for(int i=0; i<RCSAS.allSport.size();i++){
            ...
            left.add(js,BorderLayout.LINE_END);
            j.add(left);
            ...
            splocation = new JLabel(s.getLocation().toString());
            spno_venue = new JLabel(s.getNo_hall() + " " + s.getVenue()+"s");
            spprice = new JLabel("RM "+String.format("%03d", s.getPrice())+".00 /per hour");
            ...
            j.setLayout(new GridLayout(1,0));
            sport.add(j, s.getName());
        }
